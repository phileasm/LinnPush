...
inline byte getCombinedLedData(byte col, byte row) {
  ...
  do {
    ...
    // --- Modified logic for when to include layers ---
    // We want:
    //  - LED_LAYER_CUSTOM1 to be shown only in displayPushMode (and in the custom editor)
    //  - not shown in displayNormal or displaySplit/displaySplitPoint
    //
    // For all other layers, preserve the previous rule:
    //  - in normal and split-point display mode, show all layers and only show the main in other display modes

    if (layer == LED_LAYER_CUSTOM1) {
      // show custom1 only in push mode (and in editor handled earlier)
      if (displayMode == displayPushMode) {
        data = ledBuffered(layer, col, row);
      }
      // otherwise, skip this layer (leave data=0 and continue checking other layers)
    }
    else {
      // previous behavior for other layers:
