...
void initializePresetSettings() {
  ...
  for (byte n = 0; n < NUMPRESETS; ++n) {
    ...
    g.switchAssignment[SWITCH_SWITCH_1] = ASSIGNED_PUSH_MODE;
    g.switchAssignment[SWITCH_SWITCH_2] = ASSIGNED_CC_65;
    ...
    g.ccForSwitchCC65[SWITCH_SWITCH_2] = 23;
    ...
    p.split[LEFT].colorMain = COLOR_WHITE;
    p.split[LEFT].colorPlayed = COLOR_GREEN;
    ...
    p.split[RIGHT].colorPlayed = COLOR_GREEN;
...
void handlePushNewTouch(){
  midiSendControlChange(20, sensorCol-1, 1, false);
  midiSendControlChange(21, 7-sensorRow, 1, false);
  midiSendControlChange(22, 127, 1, false);
  //handleCustomSwitchAssignmentConfigNewTouch();
  //midiSendControlChange(controlnum, val, 1, false); // send cc ON, pad coordinate translating to CC number and value
}

void handlePushRelease(){
  midiSendControlChange(20, sensorCol-1, 1, false);
  midiSendControlChange(21, 7-sensorRow, 1, false);
  midiSendControlChange(22, 0, 1, false);
  //handleCustomSwitchAssignmentConfigRelease();
  //midiSendControlChange(controlnum, val, 1, false); // send cc OFF, pad coordinate translating to CC number and value
}